...
```{r}
datapath<-paste0(wd,"/data/feat_gene_age_num.txt")
gene_age<-read.table(datapath, header=TRUE)
net<-chaser::load_features(net,gene_age,type="features_table",missingv=18,auxfun = "min")
```
...
write.csv(get.vertex.attribute(ignet), file="C:/Users/Delouche Alexis/Desktop/stage/data/ignet/attributes.csv")

# opening a corresponding table  of geneage name and net1$features
datapath<-paste0(wd,"/data/table_age_gene_num.txt")
...
for (i in corresp_table[,2]){
  ...
  write.csv(tab_edge, file = paste0("C:/Users/Delouche Alexis/Desktop/stage/data/ignet",paste0("/",i,corresp_table[i,1]," network.csv")))
}
